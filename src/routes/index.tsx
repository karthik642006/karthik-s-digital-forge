...
function Portrait() { const [missing, setMissing] = useState(false); const portraitUrl = "https://github.com/karthik642006.png?size=900"; return <motion.div initial={{ opacity: 0, scale: .96 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: .25 }} className="relative mx-auto aspect-[4/5] w-full max-w-sm overflow-hidden rounded-md border border-border bg-surface shadow-2xl"><div className="absolute inset-3 border border-border"/>{!missing && <img src={portraitUrl} onError={() => setMissing(true)} alt="Portrait of Karthik" className="h-full w-full object-cover object-center"/>}{missing && <div className="grid h-full place-items-center p-8 text-center"><div><div className="mx-auto grid size-20 place-items-center rounded-full border border-primary/40 bg-accent font-mono text-2xl text-primary">KK</div><p className="mt-6 font-mono text-xs text-muted-foreground">/karthik-portrait.jpg</p><p className="mt-2 text-xs text-muted-foreground">Portrait ready for upload</p></div></div>}<div className="absolute bottom-5 left-5 right-5 flex items-center justify-between rounded-md border border-border bg-surface/85 px-4 py-3 backdrop-blur-xl"><span className="font-mono text-xs">KARTHIK.DEV</span><span className="flex items-center gap-2 text-[10px] uppercase text-primary"><span className="size-1.5 animate-pulse rounded-full bg-primary"/>Online</span></div></motion.div>; }

...
function Portrait() { const [missing, setMissing] = useState(false); return <motion.div initial={{ opacity: 0, scale: .96 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: .25 }} className="relative mx-auto aspect-[4/5] w-full max-w-sm overflow-hidden rounded-md border border-border bg-surface shadow-2xl"><div className="absolute inset-3 border border-border"/>{!missing && <img src="/karthik-portrait.jpg" onError={() => setMissing(true)} alt="Portrait of Karthik" className="h-full w-full object-cover grayscale"/>}{missing && <div className="grid h-full place-items-center p-8 text-center"><div><div className="mx-auto grid size-20 place-items-center rounded-full border border-primary/40 bg-accent font-mono text-2xl text-primary">KK</div><p className="mt-6 font-mono text-xs text-muted-foreground">/karthik-portrait.jpg</p><p className="mt-2 text-xs text-muted-foreground">Portrait ready for upload</p></div></div>}<div className="absolute bottom-5 left-5 right-5 flex items-center justify-between rounded-md border border-border bg-surface/85 px-4 py-3 backdrop-blur-xl"><span className="font-mono text-xs">KARTHIK.DEV</span><span className="flex items-center gap-2 text-[10px] uppercase text-primary"><span className="size-1.5 animate-pulse rounded-full bg-primary"/>Online</span></div></motion.div>; }
...
function ContactRow({ icon, label }: { icon: React.ReactNode; label: string }) { return <div className="flex items-center gap-3 text-sm text-muted-foreground"><span className="grid size-9 place-items-center rounded-md border border-border bg-surface [&_svg]:size-4">{icon}</span><span>{label}</span><span className="ml-auto font-mono text-[10px] uppercase">Not provided</span></div>; }

function ContactForm() { const [sent, setSent] = useState(false); const submit = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); setSent(true); }; return <form onSubmit={submit} className="border border-border bg-card p-6 sm:p-8"><div className="grid gap-4 sm:grid-cols-2"><label className="text-xs text-muted-foreground">Name<Input required name="name" className="mt-2 h-11 bg-background/40" placeholder="Your name"/></label><label className="text-xs text-muted-foreground">Email<Input required name="email" type="email" className="mt-2 h-11 bg-background/40" placeholder="you@example.com"/></label></div><label className="mt-4 block text-xs text-muted-foreground">Message<Textarea required name="message" className="mt-2 min-h-36 bg-background/40" placeholder="Tell me about the opportunity or project."/></label><Button type="submit" variant="hero" size="lg" className="mt-5 w-full">{sent ? <><Check/>Message prepared</> : <>Prepare message <Send/></>}</Button>{sent && <p role="status" className="mt-3 text-center text-xs text-muted-foreground">Add Karthik’s email address to enable direct sending.</p>}</form>; }